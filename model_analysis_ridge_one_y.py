...
2D with SOH vs cycle number. These models will be fitted to Ridge Regression. Ridge Regression will be
tested for various alpha hyperparameters [2, 5, 10, 50, 100, 200, 500, 1000, 10000].
For both the train and validation sets, the R^2 value will be calculated for each model. The model with the
highest R^2 value will be selected to proceed to the test set. The test set will be individually tested against 
the model."""
...
def create_models(train_set_X, val_set_X, train_set_y, val_set_y, headers_x, key):
    """We will test ridge regression models. Ridge will be tested for various alpha 
    values [2, 5, 10, 50, 100, 200, 500, 1000, 10000]. Create a dictionary to store the R^2 values"""
    ...
    for alpha in [2, 5, 10, 50, 100, 200, 500, 1000, 10000]:
        model_name == "ridge"
        model = linear_model.Ridge(alpha=alpha, max_iter=10000)
        alpha_value = alpha
...
def plot_figures(y_true, y_pred, key, recipe_type, target_scaler,  x_true, scaler):
    ...
    plt.savefig(f"Data/Completed_Analysis_Ridge_one_y/{recipe_type}_Model_Predictions_vs_True_Values_Cell_{int(key) + 89}.png")
...
def check_multicollinearity(data):
    ...
    # Save the figure to Data/Completed_Analysis
    plt.savefig("Data/Completed_Analysis_Ridge_one_y/Correlation_Matrix.png")
...
def main():
    ...
    for key, data in data_train_val.items():
        ...
        coefficients_df.to_csv(f"Data/Completed_Analysis_Ridge_one_y/{key}_coefficients.csv", index=False)
    ...
    chosen_model_df.to_csv("Data/Completed_Analysis_Ridge_one_y/chosen_model.csv", index=False)
    print(chosen_model_df)


    """Although this was a valiant effort, the hypothesis that Ridge Regression would be the best model for all datasets 
    does not show to be very effective as a predictor for the real driving scenarios. When inspecting the plots, the 
    general trend can be seen, but the model does not predict the exact values. This could be due to the fact that
    the real driving data is not as consistent as the other datasets. The model may need to be retrained with the real
    driving data to improve accuracy. In addition, the model may need to be more complex to capture the nuances of the
    real driving data. The model may also need to be more flexible to account for the variability in the real driving."""
