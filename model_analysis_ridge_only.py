...
def plot_figures(y_true, y_pred, key, recipe_type, target_scaler):
    ...
    plt.savefig(f"Data/Completed_Analysis_Ridge_Only/{recipe_type}_Model_Predictions_vs_True_Values_Cell_{int(key) + 89}.png")
...
def check_multicollinearity(data):
    ...
    # Save the figure to Data/Completed_Analysis
    plt.savefig("Data/Completed_Analysis_Ridge_Only/Correlation_Matrix.png")
...
def main():
    ...
    for key, data in data_train_val.items():
        ...
        coefficients_df.to_csv(f"Data/Completed_Analysis_Ridge_Only/{key}_coefficients.csv", index=False)
    ...
    chosen_model_df.to_csv("Data/Completed_Analysis_Ridge_Only/chosen_model.csv", index=False)
    ...
    was incorrect. The Ridge Regression model performed well for the periodic dataset, but not for the constant current or
    synthetic datasets. The real driving dataset was used to test the model, and the results were not highly impacted by 
    the outliers. Perhaps another model will perform better. The next step is to test the Huber Regressor model."""
