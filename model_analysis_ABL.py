...
def load_data():
    with open("Data/ProcessedData/df_constant_current_A.pkl", "rb") as f:
        data_constant_current = pickle.load(f)
    with open("Data/ProcessedData/df_periodic_A.pkl", "rb") as f:
        data_periodic = pickle.load(f)
    with open("Data/ProcessedData/df_synthetic_A.pkl", "rb") as f:
        data_synthetic = pickle.load(f)
    with open("Data/ProcessedData/df_real_driving_A.pkl", "rb") as f:
...
def create_models_cv(train_set_X, val_set_X, train_set_y, val_set_y, headers_x, seed=77):
    ...
    # Scale the y data
    target_scaler = RobustScaler()
    train_set_y_scaled = target_scaler.fit_transform(train_set_y)
    val_set_y_scaled = target_scaler.transform(val_set_y)
...
def plot_figures(y_true, y_pred, key, recipe_type, target_scaler):
    ...
    y_pred = pd.DataFrame(y_pred[:,0], columns=["Normalized Discharge Capacity [-]"])
...
def main():
    ...
    for key, data in data_train_val.items():
        ...
        coefficients_df_cv.to_csv(f"Data/Completed_Analysis/{key}_coefficients.csv", index=False)
        ...
        chosen_model_df_cv.to_csv("Data/Completed_Analysis/chosen_model.csv", index=False)
